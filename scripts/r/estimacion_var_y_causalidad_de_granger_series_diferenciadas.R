...
orden_optimo <- as.numeric(names(sort(table(unlist(lag_selection)), decreasing = TRUE))[1])

matriz_var <- ts(do.call(cbind, series_est))
modelo_var <- VAR(matriz_var, p = orden_optimo, type = "both")
summary(modelo_var)
